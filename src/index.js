...
export default class VirtualScroll {
  ...
  #touchStart;
  #bodyTouchAction;
  #isCurrentPointerEvent;
  #pointerClickPrevent;
  #pointerDownPos;

  constructor(options) {
    this.#el = window;
    this.#isCurrentPointerEvent = false;
    this.#pointerClickPrevent = null;
    ...
    this.#options = Object.assign(
      {
        ...
        useTouch: true,
        usePointer: true,
        pointerClickDelta: 1 * window.devicePixelRatio,
      },
      options
    );
  ...
  _onTouchStart = (e) => {
    ...
    if (e.type === "pointerdown") {
      this.#isCurrentPointerEvent = true;
      this.#pointerDownPos = { x: t.pageX, y: t.pageY };
      ...
      const onPointerUp = () => {
        ...
        if (this.#pointerClickPrevent) {
          setTimeout(() => {
            this.#el.removeEventListener("click", this.#pointerClickPrevent, {
              capture: true,
            });
            this.#pointerClickPrevent = null;
          });
  ...
  _onTouchMove = (e) => {
    ...
    if (this.#isCurrentPointerEvent) {
      e.preventDefault();

      if (!this.#pointerClickPrevent) {
        const distance = Math.sqrt(
          Math.pow(
            (t.pageX - this.#pointerDownPos.x) / options.touchMultiplier,
            2
          ),
          Math.pow(
            (t.pageY - this.#pointerDownPos.y) / options.touchMultiplier,
            2
          )
        );

        if (distance > this.#options.pointerClickDelta) {
          this.#pointerClickPrevent = (e) => {
            e.preventDefault();
            e.stopPropagation();
          };
          this.#el.addEventListener("click", this.#pointerClickPrevent, {
            capture: true,
          });
        }
      }
  ...
  _bind() {
    ...
    if (!support.hasTouch && this.#options.usePointer) {
